...
High Priority tasks are being scheduled first (priority_tasks())
Tasks from multiple pets get grouped accordingly (schedule_clustered_by_pet())
Cache invalidation if any task changes.(get_cached_total_time())
The total scheduled time never goes past owner's available time (is_feasible())
...
I am like 70% percent sure that my scheduler works correctly. If I had more time, I would have added a bunch more tasks to see if it would do the higher priority tasks first and also test for circular dependency to see if two classes are entirely dependent on each other.
